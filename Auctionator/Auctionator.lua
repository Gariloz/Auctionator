...
local function Atr_SlashCmdFunction(msg)
	...
	elseif (cmd == "scandiag") then

		if not AtrScanDiag_HandleCommand(param1, param2) then
			zc.msg_atr ("usage: /atr scandiag [show|clear|verbose [on|off]]");
		end
...
function Atr_OnAuctionUpdate (...)
	...
	if (gAtr_FullScanState == ATR_FS_SLOW_SCAN) then
		Atr_FullScanSlowScanProcessPage();
		return;
	end

	if (gAtr_FullScanState == ATR_FS_ANALYZING or gAtr_FullScanState == ATR_FS_GETALL_RETRY) then
		return;
	end

	if (gAtr_FullScanState == ATR_FS_STARTED) then
		if (gAtr_FullScanUseGetAll) then
			Atr_FullScanOnGetAllAuctionUpdate();
		end
		return;
	end
...
function Atr_Idle(self, elapsed)
	...
	if (Atr_FullScanRefreshUIIfNeeded) then
		Atr_FullScanRefreshUIIfNeeded (elapsed);
	end
